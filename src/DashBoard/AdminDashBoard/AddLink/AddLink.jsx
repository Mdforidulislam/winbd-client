...
const AddLink = () => {
    ...
    useEffect(() => {
        ...
        const fetchDynamicUrl = async () => {
            try {
                const response = await axios.get('https://windb-server-live-test.vercel.app/getingDynamicallyUrl');
    ...
    const handleUpdateCustomerCareNumber = async (event) => {
        ...
        try {
            const response = await axios.patch('https://windb-server-live-test.vercel.app/insertDynamiceUrl', {
                uniqueId: localData,
                redirectUrl: link
            });
