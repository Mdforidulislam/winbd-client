...
const AddLink = () => {
    ...
    useEffect(() => {
        ...
        const fetchDynamicUrl = async () => {
            try {
                const response = await axios.get('https://winbd-server-test-2.vercel.app/getingDynamicallyUrl');
    ...
    const handleUpdateCustomerCareNumber = async (event) => {
        ...
        try {
            const response = await axios.patch('https://winbd-server-test-2.vercel.app/insertDynamiceUrl', {
                uniqueId: localData,
                redirectUrl: link
            });
