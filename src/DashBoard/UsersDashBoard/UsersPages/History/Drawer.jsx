...
const Drawer = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const getingSearchValue = async () => {
      if (userName) {
        const getingResponse = await axios.get(`http://localhost:5000/userHistory?userName=${encodeURIComponent(userName)}`);
  ...
  const handleActiongetingValueGeting = async () => {
    try {
      ...
      const getingResponse = await axios.get(`http://localhost:5000/userHistory?userName=${encodeURIComponent(userName)}&${queryString}`);
      const userSearchData = getingResponse.data.data;
