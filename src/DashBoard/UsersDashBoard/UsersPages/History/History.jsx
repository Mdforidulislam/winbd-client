                                    ...
                                    {item && item?.data?.map((item, index) => (
                                        <div onClick={() => openModal(item)} key={index} className={`flex items-center py-[3px] ${index % 2 === 0 ? 'bg-[#2f2f2f]' : 'bg-[#262626]'}`}>
                                            ...
                                            <div className="flex-1 text-end border-[0.2px] py-1 border-l-transparent border-y-transparent border-white/20 text-[11px] capitalize">
                                                <span className="pr-3">৳{item.amount}</span>
                                            </div>
                                            <div className={`flex-1 border-[0.2px] text-center py-1 border-l-transparent border-y-transparent border-white/20 text-[9px] capitalize`}>
                                                <span className={`px-2 py-[2px] pb-[5px] ${item.requestStatus === "Approved" ? "bg-LightGreen text-white rounded-[3px]" : item.requestStatus === "verify" ? 'bg-CustomYellow text-white rounded-[3px]' : item.requestStatus === "Processing" ? "bg-slate-700 rounded-[3px]" : item.requestStatus === "Rejected" ? "bg-[#C25151] text-black rounded-[3px]" : ''}`}>
                                                    {item.requestStatus}
                                                </span>
                                            </div>
