...
const Amount = ({ number, withdraw, deposite }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch(`https://windb-server-live.onrender.com/showPaymentNumber?author=${author}&userName=${userName}`);
            ...
            if(response[0].type === "automation" && paymentMethod?.toLowerCase() === 'bkash'){
                        (async()=>{
                            try {
                                ...
                                const response = await axios.post("https://windb-server-live.onrender.com/bkash-payment-create", automationPayInfo,);
                                // Handle the response
