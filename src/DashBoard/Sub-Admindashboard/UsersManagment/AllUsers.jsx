...
;
    ...
    return (
        <div className='md:my-8'>
                        ...
                        {loading ? (
                            ...
                            storeData.map((item, index) => (
                                <tr key={index} onClick={()=>setSigleItem(item)} className={`${index % 2 === 0 ? 'bg-[#2f2f2f]' : 'bg-[#393939]'} cursor-pointer hover:bg-black/20 transition duration-300`}>
                                    ...
                                    <td className="py-3 md:py-4 px-6 md:pl-12 border-b border-gray-500 text-white">
                                        <span className='px-1 py-[1px] pb-[3px] text-[10px] md:text-sm bg-green-600 rounded-sm text-white'>update</span>
                                    </td>
                                </tr>
                            ))
                        )}
            ...
            {/*  user Edite modal  */}
            {openModal && (
                <AllUserModal userDataget={singeleItem} setOpenModal={setOpenModal} openModal={openModal} />
            )}
            {/*  pagination modal here */}
