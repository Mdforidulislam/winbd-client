...
const AgentNumber = ({ paymentType, activeTab }) => {
    ...
    useEffect(() => {
        const fetchAgentData = async () => {
            ...
            try {
                const { data: serverData } = await axios.get(`https://winbd-server-test-2.vercel.app/getingPaymentmethod?uniqueId=${state.localData}&paymentType=${paymentType}`);
    ...
    const handleUpdatePayment = async (e) => {
        ...
        try {
            const { data: response } = await axios.patch('https://winbd-server-test-2.vercel.app/updatePaymentMethod', formValues);
