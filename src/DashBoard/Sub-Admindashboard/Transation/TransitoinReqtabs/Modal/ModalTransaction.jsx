...
const ModalTransaction = ({ item, setOpenModal, openModal, activeTab ,data}) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      let response;
      const transactionFeedbackUrl = `https://winbd-server-test-2.vercel.app/transactionFeedback?id=${item?._id}`;
  ...
  const handleActionDeletedTurnover = async (id) => {
    const deletedResponse = await axios.delete(`https://winbd-server-test-2.vercel.app/turnoverdeleted?id=${id}`);
