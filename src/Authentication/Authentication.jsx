...
const Authentication = ({ children }) => {
  ...
  const loginUserNamePassword = async (userName, password) => {
    try {
      const res = await fetch(`https://windb-server-live.onrender.com/userValidation?userName=${userName}&password=${password}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
       if(userInfo.role){
          const res = await fetch(`https://windb-server-live.onrender.com/userValidation?userName=${userInfo.userName}&password=${userInfo.password}`);
          const data = await res.json()
          setRole(data?.role)
